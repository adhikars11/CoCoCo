# Improved Neural Network with Partial Dependence Plot and ICE Curves
# Filename: improved_nn_pdp_updated.R
...
# ------------------------
# Step 3: Neural Network Training (With Quadratic Term)
# ------------------------
...
tune_grid <- expand.grid(size = c(5, 10, 15), decay = c(0.01, 0.001, 0.1))
...
print("Cross-validated neural network training complete!")
...
test_data$nn_pred <- predict(nn_model, test_data)
...
# ------------------------
# Step 5: Partial Dependence Plot and ICE Curves
# ------------------------

# Generate Partial Dependence Plot for hhi_index with higher grid resolution
pdp_hhi <- partial(
  object = nn_model,
  pred.var = "hhi_index",
  train = as.data.frame(train_data),
  grid.resolution = 400,  # Increase the number of grid points for smoother curve
...
plot_pdp <- ggplot(pdp_hhi, aes(x = hhi_index, y = yhat)) +
  ...
  labs(title = "PDP of Log Patents on HHI Index (Higher Resolution)",
...
# Save the updated PDP Plot
pdp_plot_path <- "pdp_hhi_index_high_res.png"
ggsave(pdp_plot_path, plot = plot_pdp)
print(paste("PDP Plot with Higher Resolution saved:", pdp_plot_path))

# ------------------------
# Step 7: Save Results
# ------------------------

output_file <- "improved_nn_with_quadratic_results.csv"
